        ...
        >
          <el-form-item>
            ...
            <a
              ...
              v-if="isAuth('shitang', '导入导出')"
              icon="el-icon-download"
              href="http://localhost:8080/xiaoyuanshitangdingcan/upload/shitangMuBan.xls"
              >批量导入食堂数据模板</a
            >
            &nbsp;
            <el-upload
              ...
              :action="getActionUrl"
              :on-success="shitangUploadSuccess"
              :on-error="shitangUploadError"
...
export default {
  data() {
    return {
      ...
      form: {
        id: null,
        shitangUuidNumber: null,
        shitangName: null,
        shitangPhoto: null,
        shitangTypes: null,
        shitangAdd: null,
        shitangClicknum: null,
        shitangContent: null,
        shitangDelete: null,
        insertTime: null,
        createTime: null,
  ...
  methods: {
    chartDialog() {
      ...
      let params = {
        ...
        thisTable: {
          //当前表
          tableName: 'shitang', //当前表表名,
          sumColum: 'shitang_number', //求和字段
          date: 'insert_time', //分组日期字段
          // string : 'shitang_name',//分组字符串字段
          // types : 'shitang_types',//分组下拉框字段
        },
        // joinTable : {//级联表（可以不存在）
    ...
    getDataList() {
      ...
      params['shitangDelete'] = 1 // 逻辑删除字段 1 未删除 2 删除
    ...
    // 导入功能上传文件成功后调用导入方法
    shitangUploadSuccess(data) {
      let _this = this
      _this
        .$http({
          url: 'shitang/batchInsert?fileName=' + data.file,
          method: 'get',
        })
        .then(({ data }) => {
          if (data && data.code === 0) {
