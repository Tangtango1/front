        ...
        >
          <el-form-item>
            ...
            <a
              ...
              href="http://localhost:8080/xiaoyuanshitangdingcan/upload/shangjiaMuBan.xls"
              >批量导入商家数据模板</a
            >
            &nbsp;
            <el-upload
  ...
  methods: {
    chartDialog() {
      ...
      let params = {
        ...
        thisTable: {
          //当前表
          tableName: 'shangjia', //当前表表名,
          sumColum: 'shangjia_number', //求和字段
          date: 'insert_time', //分组日期字段
          // string : 'shangjia_name',//分组字符串字段
          // types : 'shangjia_types',//分组下拉框字段
        },
        // joinTable : {//级联表（可以不存在）
    ...
    resetPassword(id) {
      ...
      //重置密码
      this.$http({
        url: 'shangjia/resetPassword?id=' + id,
        method: 'get',
        // id:id
      }).then(({ data }) => {
        if (data && data.code === 0) {
