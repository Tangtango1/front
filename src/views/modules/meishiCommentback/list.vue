    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        >
          <el-form-item>
            <el-button
              v-if="isAuth('meishiCommentback', '新增')"
              type="success"
              :icon="ElIconPlus"
            ...
            &nbsp;
            <el-button
              v-if="isAuth('meishiCommentback', '删除')"
            ...
            &nbsp;
            <el-button
              v-if="isAuth('meishiCommentback', '报表')"
              type="success"
              :icon="ElIconPieChart"
              @click="chartDialog()"
              >报表</el-button
            ...
            <a
              ...
              v-if="isAuth('meishiCommentback', '导入导出')"
              icon="el-icon-download"
              href="http://localhost:8080/xiaoyuanshitangdingcan/upload/meishiCommentbackMuBan.xls"
              >批量导入美食评价数据模板</a
            >
            &nbsp;
            <el-upload
              ...
              :action="getActionUrl"
              :on-success="meishiCommentbackUploadSuccess"
              :on-error="meishiCommentbackUploadError"
      ...
      <div class="table-content">
        <el-table
          ...
          v-if="isAuth('meishiCommentback', '查看')"
          :data="dataList"
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <span
                v-if="
                  scope.row.meishiCommentbackText != null &&
                  scope.row.meishiCommentbackText.length > 10
                "
              >
                {{ scope.row.meishiCommentbackText.slice(0, 10) }}...
              </span>
              <span v-else>
                {{ scope.row.meishiCommentbackText }}
              </span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
              <span
                v-if="
                  scope.row.replyText != null && scope.row.replyText.length > 10
                "
              >
  ...
  methods: {
    chartDialog() {
      ...
      let params = {
        ...
        thisTable: {
          //当前表
          tableName: 'meishi_commentback', //当前表表名,
          sumColum: 'meishi_commentback_number', //求和字段
          date: 'insert_time', //分组日期字段
          // string : 'meishi_commentback_name',//分组字符串字段
          // types : 'meishi_commentback_types',//分组下拉框字段
        },
        // joinTable : {//级联表（可以不存在）
      ...
      //         groupColumn: "meishi_commentback_types",
      //     }
      //     this.$http({
    ...
    getDataList() {
      ...
      params['meishiCommentbackDelete'] = 1 // 逻辑删除字段 1 未删除 2 删除

      this.$http({
        url: 'meishiCommentback/page',
        method: 'get',
        params: params,
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    // 导入功能上传文件成功后调用导入方法
    meishiCommentbackUploadSuccess(data) {
      let _this = this
      _this
        .$http({
          url: 'meishiCommentback/batchInsert?fileName=' + data.file,
          method: 'get',
        })
        .then(({ data }) => {
          if (data && data.code === 0) {
    ...
    // 导入功能上传文件失败后调用导入方法
    meishiCommentbackUploadError(data) {
    ...
    wuyong(id) {
      ...
      _this
        ...
        .then(() => {
          _this
            .$http({
              url: 'meishiCommentback/update',
              method: 'post',
              data: {
                id: id,
                //                            meishiCommentbackTypes:meishiCommentbackTypes,
              },
            })
            .then(({ data }) => {
              if (data && data.code === 0) {
