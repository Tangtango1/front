        ...
        >
          <el-form-item>
            ...
            <a
              ...
              href="http://localhost:8080/xiaoyuanshitangdingcan/upload/yonghuMuBan.xls"
              >批量导入用户数据模板</a
            >
            &nbsp;
            <el-upload
        ...
        >
          ...
          >
            <template slot-scope="scope">
              ...
              >
              <el-button
                v-if="isAuth('yonghu', '状态')"
                type="success"
                size="mini"
                @click="zhanghao(scope.row.id, scope.row.statusTypes)"
                >{{
                  scope.row.statusTypes == 1 ? '冻结账号' : '取消冻结'
                }}</el-button
              >
              <el-button
                v-if="isAuth('yonghu', '查看')"
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      if (
        this.searchForm.sexTypes != '' &&
        this.searchForm.sexTypes != undefined
      ) {
    ...
    zhanghao(id, types) {
      let data = {
        id: id,
        statusTypes: types == 1 ? 2 : 1,
      }
      this.$confirm(
        `确定进行[${types == 1 ? '冻结账号' : '取消冻结'}]操作?`,
        '提示',
        {
