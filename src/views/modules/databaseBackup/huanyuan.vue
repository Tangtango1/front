  ...
  <div class="addEdit-block">
    ...
    >
      <el-form-item class="input" label="MySQL安装目录" prop="mysqlUrl">
        <el-input
          clearable
          placeholder="MySQL安装目录"
          v-model="ruleForm.mysqlUrl"
        ></el-input>
      </el-form-item>
      <el-form-item
        class="input"
        label="MySQL数据库所在服务器地址IP"
        prop="hostIP"
      >
        <el-input
          clearable
          placeholder="MySQL数据库所在服务器地址IP"
          v-model="ruleForm.hostIP"
        ></el-input>
      </el-form-item>
      <el-form-item
        class="input"
      ...
      <el-form-item class="input" label="数据库文件保存路径" prop="savePath">
        <el-input
          clearable
          placeholder="数据库文件保存路径"
          v-model="ruleForm.savePath"
        >
        </el-input>
      </el-form-item>
      <el-form-item
        class="input"
        label="数据库还原文件文件名(.sql结尾)"
        prop="fileName"
      >
        <el-input
          clearable
          placeholder="数据库还原文件文件名"
      ...
      <el-form-item class="input" label="要还原的数据库名" prop="databaseName">
        <el-input
          clearable
          placeholder="要还原的数据库名"
          v-model="ruleForm.databaseName"
        ></el-input>
      </el-form-item>
      <el-form-item class="btn">
        <el-button type="primary" class="btn-success" @click="huanyuan"
          >确定</el-button
        >
      </el-form-item>
    </el-form>
        ...
        if (valid) {
          _this
            .$http({
              url:
                '/huanyuan?hostIP=' +
                _this.ruleForm.hostIP +
                '&userName=' +
                _this.ruleForm.userName +
                '&hostPort=' +
                _this.ruleForm.hostPort +
                '&password=' +
                _this.ruleForm.password +
                '&savePath=' +
                _this.ruleForm.savePath +
                '&fileName=' +
                _this.ruleForm.fileName +
                '&databaseName=' +
                _this.ruleForm.databaseName +
                '&mysqlUrl=' +
                _this.ruleForm.mysqlUrl,
              method: 'get',
            })
            .then(({ data }) => {
              if (data && data.code === 0) {
